...
void ResourceManager::unloadAllRecources()
{
...
std::shared_ptr<RenderEngine::ShaderProgram> ResourceManager::getShaderProgram(const std::string &shaderName)
{
    ...
    std::cerr << "Error get Shader: " << shaderName << std::endl;
    return nullptr;
}
...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::loadTextures(const std::string &textureName, const std::string texturePath)
{
    ...
    if (!pixels)
    {
        std::cerr << "Can't load img: " << textureName << std::endl;
        return nullptr;
    }
...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::getTexture(const std::string &textureName)
{
    ...
    std::cerr << "Error get Texture: " << textureName << std::endl;
    return nullptr;
}
...
bool ResourceManager::loadJSONResources(const std::string &JSONPath)
{
    ...
    rapidjson::ParseResult parsResult = document.Parse(JSONString.c_str());
    if (!parsResult)
    {
        std::cerr << "JSON parse error:" << rapidjson::GetParseError_En(parsResult.Code()) << "(" << parsResult.Offset() << ")" << std::endl;
    ...
    if (shadersIt != document.MemberEnd())
    {
        for (const auto &currentShader : shadersIt->value.GetArray())
        {
            ...
            const std::string filePath_v = currentShader["filePath-v"].GetString();
            const std::string filePath_f = currentShader["filePath-f"].GetString();
    ...
    if (textureAtlasesIt != document.MemberEnd())
    {
        for (const auto &currenttextureAtlas : textureAtlasesIt->value.GetArray())
        {
            ...
            for (auto &currentSubTexture : subTexturesArray)
            {
